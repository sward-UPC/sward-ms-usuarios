...
def use_case():
    ...
    token_port.generar_par.return_value = TokenPair(access_token="access.jwt", refresh_token="refresh.jwt")
    cache = AsyncMock()
...
async def test_login_exitoso(use_case):
    result = await use_case.execute(AutenticarUsuarioCommand(correo="test@upc.edu.pe", password="Password1"))
