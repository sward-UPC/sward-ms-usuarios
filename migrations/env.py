"""Entorno Alembic (async) para sward-ms-usuarios.

- La URL se toma de `settings.database_url` (postgresql+asyncpg), igual que la app.
- `version_table = alembic_version_usuarios`: cada microservicio versiona por
...
# Base.metadata (audit_log, notification, role definen tablas sobre el mismo Base).
...
VERSION_TABLE = "alembic_version_usuarios"
